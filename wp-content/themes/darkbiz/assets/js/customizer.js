...
!function (t, n) {
  n.sectionConstructor["darkbiz-anchor"] = n.Section.extend({
...
wp.customize.controlConstructor['darkbiz-buttonset'] = wp.customize.Control.extend({
  ...
  jQuery(document).ready(function ($) {
    ...
    $('.alpha-color-control').each(function () {
      ...
      colorPickerOptions = {
        ...
        palettes: darkbizColorPalette.colorPalettes // Use the passed in palette.
      ...
      $('<div class="alpha-darkbiz-color-picker-container">' + '<div class="min-click-zone click-zone"></div>' + '<div class="max-click-zone click-zone"></div>' + '<div class="alpha-slider"></div>' + '<div class="transparency"></div>' + '</div>').appendTo($container.find('.wp-picker-holder'));
...
wp.customize.controlConstructor['darkbiz-dimensions'] = wp.customize.Control.extend({
  ready: function ready() {
    'use strict';
...
jQuery(document).ready(function ($) {
  ...
  $('.linked').on('click', function () {
    ...
    $this.parent('.link-darkbiz-dimensions').removeClass('unlinked');
  }); // Unlinked button
  ...
  $('.unlinked').on('click', function () {
    ...
    $this.parent('.link-darkbiz-dimensions').addClass('unlinked');
...
wp.customize.controlConstructor['darkbiz-icon'] = wp.customize.Control.extend({
  ready: function ready() {
    'use strict';
...
jQuery(window).load(function () {
  ...
  jQuery('.customize-control-darkbiz-radio-image .buttonset').buttonset(); // Handles setting the new value in the customizer.

  jQuery('.customize-control-darkbiz-radio-image input:radio').change(function () {
...
wp.customize.controlConstructor['darkbiz-range'] = wp.customize.Control.extend({
  ready: function ready() {
    'use strict';
    ...
    var control = this,
        ...
        input_default,
        changeAction,
        darkbiz_range_input_number_timeout; // Update the text value
    ...
    function autocorrect_range_input_number(input_number, timeout) {
      ...
      clearTimeout(darkbiz_range_input_number_timeout);
      darkbiz_range_input_number_timeout = setTimeout(function () {
        if (isNaN(value)) {
...
wp.customize.controlConstructor['darkbiz-section-order'] = wp.customize.Control.extend({
  ready: function ready() {
    'use strict';

    var control = this;
    jQuery('#customize-control-' + control.params.id).find('.darkbiz-sortable-section').sortable({
...
wp.customize.controlConstructor['darkbiz-slider'] = wp.customize.Control.extend({
  ready: function ready() {
    'use strict';
...
(function ($, api) {
  api.controlConstructor['darkbiz-toggle'] = api.Control.extend({
    ready: function ready() {
      var control = this;
